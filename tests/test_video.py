...
from vexy_lines_run.video import VideoInfo, probe, process_video, process_video_with_style, svg_to_pil
...
class TestProbe:
    def test_nonexistent_file_raises_or_returns_empty(self):
        """probe() on a nonexistent file should either raise an exception or return VideoInfo with total_frames == 0."""
        raised = False
        result = None
        try:
            result = probe("/nonexistent/video.mp4")
        except Exception:
            raised = True
        assert raised or (result is not None and result.total_frames == 0), (
            "probe() on a nonexistent file should raise or return VideoInfo with total_frames == 0"
        )
