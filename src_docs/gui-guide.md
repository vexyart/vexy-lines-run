...
The menu bar mirrors the buttons in the GUI, so you can use menus or buttons interchangeably.
...
Supported formats: PNG, JPG, JPEG, GIF, BMP, TIFF, WEBP, or anything else Pillow can open.

A style **must** be selected for image export. Without one, there's nothing to apply. The app opens each image as a new document in Vexy Lines through the MCP API and replicates the style's fill tree onto it. The engine renders, and the app exports the result.
...
5. **Export**: The app extracts each frame, saves it as a temporary PNG, sends it to the Vexy Lines engine via MCP for style transfer, rasterises the resulting SVG, and writes the output. For MP4, frames go through OpenCV's VideoWriter. For individual images, each frame is saved directly.
...
When the Lines tab is active, the style panel is disabled (grayed out). Lines files carry their own fill structure, so the app disables the style picker to avoid confusion. Switch to Images or Video to enable the style picker.
...
- **All other formats:** Folder dialog. Choose a directory; the app names files automatically after the input filenames
...
The app filters by extension based on the drop target. Drop a PNG on the Lines list and nothing happens: no error, no action.
...
The app silently ignores duplicate files: drop the same image twice and it appears only once in the list.

Drag-and-drop requires tkinterdnd2, which is included in the base install. If it's missing (e.g. on some Linux minimal installs), the app still works; you use the file dialogs and menus instead.
...
- Use 1x size, because larger multipliers increase rasterisation time
...
Previews are extracted from the `.lines` file's embedded `PreviewDoc` data (base64-encoded). If the `.lines` file was saved without a preview, or the preview data is corrupt, the preview area stays blank. This doesn't affect export, because the style and source image data are separate from the preview.
