...
Without ffmpeg, video export still works, but without audio passthrough.
...
Drag-and-drop depends on the Tcl/Tk DnD extension. On some minimal Linux installs, the underlying `tkdnd` library may be missing even if `tkinterdnd2` is installed. The app works without it: you use the buttons and menus instead.
...
The test suite mocks the MCP client and macOS APIs, so tests run on any platform without the Vexy Lines app.
